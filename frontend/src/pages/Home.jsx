...
const Section = ({ title, badge, items, loading }) => (
  <div className="px-4 sm:px-8 lg:px-16 mt-2 pb-10">
    <div className="flex items-center gap-3 mb-4">
      ...
      {badge && (
        <span
          className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${badge.color} text-white animate-pulse`}
        >
          {badge.text}
        </span>
      )}
...
const Home = () => {
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    setError(null);
    // Prefer Kitsu for anime, with TMDB as a fallback.
    ...
    const animeFallback = values[8];
    const nextData = {
      trendingAnime: (values[0].length ? values[0] : animeFallback).slice(
        0,
        15,
      ),
      classicAnime: (values[1].length ? values[1] : animeFallback).slice(0, 15),
      popularAnime: (values[2].length ? values[2] : animeFallback).slice(0, 15),
    ...
    setHero(
      [...nextData.trendingAnime, ...nextData.movies, ...nextData.tv]
        .filter((i) => i.backdrop)
        .slice(0, 8),
    );

    const hasAnyItems = Object.values(nextData).some(
    ...
    <div className="bg-[#0a0a0a] text-white min-h-screen">
      ...
      <div className="pt-[60px] md:pt-[68px]">
        <HeroSection
          ...
          loading={loading}
          title="MovieHub"
          subtitle="Watch trending anime, movies & web series"
        />
