...
const Badge = ({ children, color = "bg-white/10" }) => (
  <span
    className={`${color} px-3 py-1 rounded-full text-xs font-medium text-white border border-white/10`}
  >
    {children}
  </span>
);
...
const MediaDetail = () => {
  ...
  const { toggleWatchlist, inWatchlist, lists, addToList, user } = useApp();
    ...
    return (
      <div className="bg-[#0a0a0a] min-h-screen flex items-center justify-center text-white">
        <div className="text-center">
          ...
          <button
            onClick={() => navigate(-1)}
            className="bg-red-600 hover:bg-red-500 text-white px-6 py-2.5 rounded-full font-semibold transition"
          >
            ← Go Back
          </button>
        </div>
  ...
  const backdrop = details.backdrop_path
    ? IMG.orig + details.backdrop_path
    : "";
  ...
  return (
    <div className="bg-[#0a0a0a] text-white min-h-screen">
      ...
      <div className="relative w-full h-[45vh] sm:h-[55vh] overflow-hidden pt-[60px] md:pt-[68px]">
        ...
        <button
          onClick={() => navigate(-1)}
          className="absolute top-4 left-4 sm:left-8 z-10 bg-black/50 backdrop-blur-sm hover:bg-black/80 border border-white/20 px-4 py-2 rounded-full text-sm font-semibold transition"
        >
          ← Back
        </button>
      </div>
        ...
        <div className="flex flex-col sm:flex-row gap-6 sm:gap-10">
          ...
          {poster && (
            <div className="shrink-0 mx-auto sm:mx-0">
              <img
                src={poster}
                alt={title}
                className="w-36 sm:w-48 md:w-56 rounded-2xl shadow-2xl ring-2 ring-white/10"
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-wrap items-center gap-2 text-sm text-gray-400 mb-4">
              ...
              {details.status && (
                <span className="bg-green-800/40 text-green-300 border border-green-700/40 px-2 py-0.5 rounded-full text-xs">
                  {details.status}
                </span>
              )}
            </div>
            ...
            <div className="flex flex-wrap gap-3 mb-6">
              {trailer && (
                <button
                  ...
                  className="flex items-center gap-2 bg-white text-black px-5 py-2.5 rounded-full font-bold text-sm hover:bg-gray-200 transition"
                >
                  ▶ Trailer
                </button>
              )}
              {imdbLink && (
                <a
                  ...
                  className="flex items-center gap-2 bg-yellow-500 text-black px-5 py-2.5 rounded-full font-bold text-sm hover:bg-yellow-400 transition"
                >
                  IMDb ↗
                </a>
              )}
              <button
                onClick={() => toggleWatchlist(thisItem)}
                ...
                <div className="relative group">
                  ...
                  <div className="absolute left-0 top-full mt-1 w-52 bg-gray-900 border border-white/10 rounded-2xl shadow-2xl overflow-hidden z-20 hidden group-hover:block">
                    {lists.map((l) => (
                      <button
                        key={l.id}
                        onClick={() => addToList(l.id, thisItem)}
              ...
              <div>
                ...
                {details.origin_country?.length > 0 && (
                  <p className="text-gray-400 text-sm mt-3">
                    Country: {details.origin_country.join(", ")}
                  </p>
                )}
                ...
                {crew.length > 0 && (
                  <div className="mt-4">
                    {crew.map((c) => (
                      <p key={c.id + c.job} className="text-sm text-gray-400">
                  ...
                  <>
                        ...
                        <div className="flex flex-wrap gap-3">
                          {providers.flatrate.map((p) => (
                            ...
                            >
                              ...
                              />
                              <span className="text-[9px] text-gray-400 text-center w-14 truncate">
                                {p.provider_name}
                              </span>
                            </div>
                          ))}
                    ...
                    {!providers.flatrate?.length && !providers.rent?.length && (
                      <p className="text-gray-500 text-sm">
                        Not available for streaming in India yet.
                      </p>
                    )}
                  </>
                ) : (
      ...
      {showTrailer && trailer && (
        ...
        >
          ...
          >
            <button
              onClick={() => setShowTrailer(false)}
              className="absolute -top-10 right-0 text-white text-lg font-bold hover:text-gray-300"
            >
              ✕ Close
            </button>
