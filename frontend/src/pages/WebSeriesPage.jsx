...
const WebSeriesPage = () => {
  ...
  const loadSeries = useCallback(async () => {
    ...
    const settled = await Promise.allSettled([
      ...
      fetchKDrama(),
      Promise.allSettled(
        internationalShows.map(async (item) => {
  ...
  const hasAnyItems =
    trending.length || topRated.length || kdrama.length || intl.length;
  const sections = [
    {
    ...
    <div className="bg-[#0a0a0a] text-white min-h-screen">
      ...
      <div className="pt-[60px] md:pt-[68px]">
        <HeroSection
          ...
          loading={loading}
          title="Web Series"
          subtitle="Trending shows, K-Dramas, and global picks"
        />
            ...
            sections.map(({ title, items, badge }) => (
              <div key={title} className="px-4 sm:px-8 lg:px-16 pb-10">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  {badge && (
                    <span
                      className={`${badge.color} text-[10px] font-bold px-2 py-0.5 rounded-full text-white animate-pulse`}
                    >
                      {badge.text}
                    </span>
                  )}
