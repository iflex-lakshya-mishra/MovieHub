...
const HeroSection = ({
  items = [],
  title = "MovieHub",
  subtitle = "Watch movies, anime & web series",
  loading = false,
}) => {
  const [current, setCurrent] = useState(0);
  const [showTrailer, setShowTrailer] = useState(false);
  const navigate = useNavigate();
  const { toggleWatchlist, inWatchlist } = useApp();
    ...
    <div className="relative w-full h-[55vh] sm:h-[65vh] lg:h-[80vh] overflow-hidden">
      ...
      <div className="absolute inset-0 flex flex-col justify-end pb-12 sm:pb-16 px-6 sm:px-12 lg:px-20 z-10 max-w-3xl">
        <div className="flex items-center gap-2 mb-3">
          <span
            className={`text-xs font-bold px-2 py-0.5 rounded-full ${item.media_type === "movie" ? "bg-blue-600" : "bg-purple-600"} text-white`}
          >
        ...
        {item.overview && (
          <p className="text-gray-300 text-sm sm:text-base line-clamp-2 max-w-lg mb-6">
            {item.overview}
          </p>
        )}
